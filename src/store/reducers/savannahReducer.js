import {
  ...
  GAME_END,
  RESET_GAME,
  DIFFICULTY_CHANGE, REMOVE_WORD, LEVEL_UP, ADD_POINTS, RESET_POINTS,
...
const savannahReducer = (state = defaultState, action) => {
    switch (action.type) {
      ...
      case ADD_POINTS: {
        return {
          ...state,
          gamePoints: (state.gamePoints === 100) ? 100 : state.gamePoints + action.points,
          points: state.points + action.points,
        };
      }
      case RESET_POINTS: {
        return {...state, gamePoints: 0};
      }
      default: {
        return {...state};
      }
